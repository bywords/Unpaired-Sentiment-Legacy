...
def sample_output(embedding, embedding_dec, output_projection=None,
  ...
  def loop_given_function(prev, i):
      return tf.cond(tf.less(i,2),
                     lambda: loop_function(prev, i),
                     lambda: loop_function_max(prev, i))
...
class Generator(object):
  ...
  def _add_encoder(self, encoder_inputs, seq_len):
    # encoder_inputs: [hps.batch_size, hps.max_enc_steps, emb_size]
    # seq_len: [hps.batch_size]
    # return last hidden state
    with tf.variable_scope('encoder'):
      cell_fw = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)
      (encoder_outputs, (fw_st)) = tf.nn.dynamic_rnn(cell_fw, encoder_inputs, dtype=tf.float32,
  ...
  def add_positive_decoder(self, embedding, emb_dec_inputs, vsize, hps):
    '''
    emb_dec_inputs: list of tensors of [hps.batch_size, hps.emb_dim]
    '''
    with tf.variable_scope('positive_decoder'):
      ...
      decoder_outputs_pretrain, decoder_outputs_max_generator = \
          self._add_decoder(loop_function=loop_function,
                            loop_function_max=loop_function_max,
                            input=emb_dec_inputs)
  ...
  def add_negetive_decoder(self, embedding, emb_dec_inputs, vsize, hps):
        '''
        Negative score일 경우 negative decoder 붙임
        '''
        with tf.variable_scope('negetive_decoder'):
            ...
            with tf.variable_scope('decoder'):
                # loop_given_function은 의미 없는 듯 - 이후 사용되지 않음
                loop_function, loop_function_max, loop_given_function = \
                    sample_output(embedding, emb_dec_inputs, (w, v))
            # 진짜 decoder를 붙이는 부분 - 두가지 종류의 output을 return 한다.
            # loop_function은 보니까 메소드 내에서 쓰지 않는 듯.
            decoder_outputs_pretrain, decoder_outputs_max_generator = \
                self._add_decoder(loop_function=loop_function,
                                  loop_function_max=loop_function_max,
                                  input=emb_dec_inputs)

            # decoder ouput을 [batch * step, hidden_dim]으로 바꾸고 decoder output * W + b를 한다.
            # W - [hidden_dim, vsize]
            decoder_outputs_pretrain = tf.reshape(decoder_outputs_pretrain,
                                                  [hps.batch_size * hps.max_dec_steps, hps.hidden_dim])
            decoder_outputs_pretrain = tf.nn.xw_plus_b(decoder_outputs_pretrain, w, v)
  ...
  def add_decoder(self,embedding, emb_dec_inputs, vsize, hps):
      # emb_dec_inputs: list of tensors of [hps.batch_size, hps.emb_dim]
      return tf.cond(tf.less(self.score, 1),
                     lambda: self.add_negetive_decoder(embedding, emb_dec_inputs, vsize, hps),
                     lambda: self.add_positive_decoder(embedding, emb_dec_inputs, vsize, hps),)

  # Generator model 구성 - 핵심부분
  def _build_model(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      # LSTM-based encoder last hidden state를 decoder의 initial state로 연결
      with tf.variable_scope('embedding'):
        embedding = tf.get_variable('embedding', [vsize, hps.emb_dim],
                                    dtype=tf.float32, initializer=self.trunc_norm_init)
        # emb_dec_inputs: [hps.batch_size, hps.max_dec_steps, hps.emb_dim]
        emb_dec_inputs = tf.nn.embedding_lookup(embedding, self._dec_batch)
        # emb_dec_inputs: list of tensors of [hps.batch_size, hps.emb_dim]. each tensor represents each step
        emb_dec_inputs = tf.unstack(emb_dec_inputs, axis=1)

        # emb_enc_inputs: [hps.batch_size, hps.max_enc_steps, emb_size]
        emb_enc_inputs = tf.nn.embedding_lookup(embedding, self._enc_batch)
        # emb_enc_inputs: [hps.batch_size, hps.max_enc_steps, emb_size]
        # self_weight: [hps.batch_size, hps.max_enc_steps] --> [hps.batch_size, hps.max_enc_steps, 1]
        # emb_enc_inputs: [hps.batch_size, hps.max_enc_steps, emb_size]
        # weight이 0인 곳은 emb vector 값이 0이 되도록 만듬
        emb_enc_inputs = emb_enc_inputs * tf.expand_dims(self._weight, axis=-1)
        # LSTM-based RNN
        # fw_st: [hps.batch_size, hps.hidden_dim]
        fw_st = self._add_encoder(emb_enc_inputs, self._enc_lens)
        # initialize first hidden state of decoder as the last hidden state of encoder
      ...
      # 어떤 score에 해당하느냐에 따라 다른 종류의 decoder를 붙인다.
      # If score is positive (==1), use negative decoder.
      # If score is negative (==0), use positive decoder.
      self.decoder_outputs_pretrain, self._max_best_output = \
          self.add_decoder(embedding, emb_dec_inputs, vsize, hps)

      loss = tf.contrib.seq2seq.sequence_loss(
          self.decoder_outputs_pretrain,  # [hps.batch_size, hps.max_dec_steps, vsize]
          self._target_batch,             # [hps.batch_size, hps.max_dec_steps]
  ...
  def build_graph(self):
    ...
    with tf.device("/gpu:"+str(FLAGS.gpuid)):
      ...
      # 아래는 training operator - gradient clipping 등
      self._add_train_op()
